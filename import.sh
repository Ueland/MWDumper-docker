...
do
	...
	docker run --rm --name mwdumper-$(echo $url | cut -d'-' -f6 | cut -f1 -d'.') -e MYSQL_DATABASE="${MYSQL_DATABASE}" -e MYSQL_HOST=${MYSQL_HOST} -e MYSQL_PASSWORD=${MYSQL_PASSWORD} -e DUMP_URL="$url" axisofentropy/mwdumper-docker
done
